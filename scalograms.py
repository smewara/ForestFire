...
plt.subplot(1, 4, 1)
librosa.display.specshow(spectrogram2, sr=sample_rate2, x_axis='time', y_axis='linear', cmap='viridis')
plt.colorbar(format='%+2.0f dB')
plt.xlabel('Time (s)')
plt.ylabel('Frequency (Hz)')
plt.title('STFT Spectrogram - Rainforest background')

# Plot Mel Spectrogram
plt.subplot(1, 4, 2)
mel_spectrogram = librosa.feature.melspectrogram(y=audio_data2, sr=sample_rate2, n_mels=128, hop_length=512, n_fft=2048)
librosa.display.specshow(librosa.amplitude_to_db(mel_spectrogram, ref=np.max), sr=sample_rate2, x_axis='time', y_axis='mel', cmap='viridis')
plt.colorbar(format='%+2.0f dB')
plt.xlabel('Time (s)')
plt.ylabel('Frequency (Hz)')
plt.title('Mel Spectrogram - Rainforest background')
...
plt.xlabel('Time (s)')
plt.ylabel('Scale')
plt.title('Morlet Wavelet Scalogram - Rainforest background')

# Plot MFCC
plt.subplot(1, 4, 4)  
mfccs2 = librosa.feature.mfcc(y=audio_data2, sr=sample_rate2, n_mfcc=10, hop_length=1, n_fft=1024)
librosa.display.specshow(mfccs2, sr=sample_rate2, x_axis='time', cmap='viridis')
plt.colorbar()
plt.title('MFCC - Rainforest background')
